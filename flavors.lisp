...
(cl:defmethod describe-object ((flavor flavor) stream)
  (when flavor
    (cond ((flavor-dependents flavor)
	   (terpri stream)
	   (princ "It is a direct component of flavors: " stream)
	   (princ (mapcar #'flavor-name (flavor-dependents flavor)) stream))
	  (t (terpri stream)
             (princ "No flavors currently refer to it." stream)))
    (Cond ((not (flavor-defined-p flavor))
	   (terpri stream)
	   (princ "It's hasn't been defflavored yet." stream))
	  (t (terpri)
	     (princ "It was defflavored something like this:" stream)
	     (terpri stream)
	     (write (reconstruct-defflavor (flavor-name flavor)) :pretty t
		    :length nil :level nil :stream stream)
	     (when (flavor-compiled-p flavor)
	       (cond ((flavor-dirty-p flavor)
		      (terpri stream) (princ "It's compiled, but dirty." stream))
		     (t (terpri stream) (princ "It's currently compiled." stream)))
	       (cond ((flavor-wayward-p flavor)
		      (terpri stream) (princ "It has wayward instances." stream))
		     ((flavor-instantiated-p flavor)
		      (terpri stream) (princ "It is instantiated." stream))))))))
